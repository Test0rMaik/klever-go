...
// from the provided ContractCallInput.
...
func copyKDATransfer(kdaTransfer *vmcommon.KDATransfer) *vmcommon.KDATransfer {
	newKDATransfer := kdaTransfer.Clone()
	return &newKDATransfer
}
...
func (context *runtimeContext) SetVMInput(vmInput *vmcommon.ContractCallInput) {
	...
	for i, kdaTransfer := range vmInput.KDATransfers {
		context.vmInput.KDATransfers[i] = copyKDATransfer(kdaTransfer)
	}
	...
	context.vmInput.KDATransfers = make([]*vmcommon.KDATransfer, len(vmInput.KDATransfers))
...
func (context *runtimeContext) FailExecution(err error) {
	context.host.Output().SetReturnCode(vmcommon.VMExecutionFailed)
...
func (context *runtimeContext) SignalUserError(message string) {
	context.host.Output().SetReturnCode(vmcommon.VMUserError)
	context.host.Output().SetReturnMessage(message)
