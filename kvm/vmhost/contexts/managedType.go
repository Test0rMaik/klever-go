...
// structure for txContracts where scA call scB and scB makes txContracts without execution to scA
type backTXContracts struct {
	TXContracts []*transaction.TXContract
	CallValue   *big.Int
}
...
type managedTypesState struct {
	...
	backTXContracts backTXContracts
}
...
func (context *managedTypesContext) NewBigInt(value *big.Int) int32 {
	if value == nil {
		return context.newBigIntNoCopy(big.NewInt(0))
	}
...
func (context *managedTypesContext) AddBackTransfers(transfers []*vmcommon.KDATransfer) {
	context.managedTypesValues.backTransfers.KDATransfers = append(context.managedTypesValues.backTransfers.KDATransfers, transfers...)
}
...
func (context *managedTypesContext) cloneBackTransfers() backTransfers {
	...
	newBackTransfers := backTransfers{
		KDATransfers: make([]*vmcommon.KDATransfer, len(currentBackTransfers.KDATransfers)),
		CallValue:    big.NewInt(0).Set(currentBackTransfers.CallValue),
	}

	for index, transfer := range currentBackTransfers.KDATransfers {
		t := transfer.Clone()
		newBackTransfers.KDATransfers[index] = &t
	}
...
// AddBackTransfers add transfers to back transfers structure
func (context *managedTypesContext) AddBackTXContracts(txContracts []*transaction.TXContract) {
	context.managedTypesValues.backTXContracts.TXContracts = append(context.managedTypesValues.backTXContracts.TXContracts, txContracts...)
}

// AddValueOnlyBackTransfer add to back transfer value
func (context *managedTypesContext) AddValueOnlyBackTXContracts(value *big.Int) {
	context.managedTypesValues.backTXContracts.CallValue.Add(context.managedTypesValues.backTXContracts.CallValue, value)
}

// GetBackTransfers returns all KDA transfers and accumulated value as well, will clean accumulated values
func (context *managedTypesContext) GetBackTXContracts() ([]*transaction.TXContract, *big.Int) {
	clonedTransfers := context.cloneBackTXContracts()
	context.managedTypesValues.backTXContracts = backTXContracts{
		TXContracts: make([]*transaction.TXContract, 0),
		CallValue:   big.NewInt(0),
	}

	return clonedTransfers.TXContracts, clonedTransfers.CallValue
}

func (context *managedTypesContext) cloneBackTXContracts() backTXContracts {
	currentBackTXContracts := context.managedTypesValues.backTXContracts

	newBackTransfers := backTXContracts{
		TXContracts: make([]*transaction.TXContract, len(currentBackTXContracts.TXContracts)),
		CallValue:   big.NewInt(0).Set(currentBackTXContracts.CallValue),
	}

	for index, transfer := range currentBackTXContracts.TXContracts {
		newBackTransfers.TXContracts[index] = proto.Clone(transfer).(*transaction.TXContract)
	}
