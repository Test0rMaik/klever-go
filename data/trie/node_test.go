...
func TestNode_getNodeFromDBAndDecodeLeafNode(t *testing.T) {
	...
	assert.True(t, proto.Equal(ln, node.(*leafNode)))
}
...
func finalizeTrieState(t *testing.T, index int, tr data.Trie, rootHashes [][]byte) {
	tr.GetStorageManager().Prune(rootHashes[index-1], data.OldRoot)
	tr.GetStorageManager().CancelPrune(rootHashes[index], data.NewRoot)
...
func rollbackTrieState(t *testing.T, index int, tr data.Trie, rootHashes [][]byte) {
	tr.GetStorageManager().Prune(rootHashes[index], data.NewRoot)
	tr.GetStorageManager().CancelPrune(rootHashes[index-1], data.OldRoot)
	time.Sleep(pruningDelay)
...
func TestPatriciaMerkleTrie_RecreateFromSnapshotSavesStateToMainDb(t *testing.T) {
	...
	tr.trieStorage.Prune(rootHash, data.NewRoot)
	...
	assert.True(t, newTr.GetStorageManager().Database() != newTsm.snapshots[0])
	val, err = newTr.GetStorageManager().Database().Get(rootHash)
	assert.Nil(t, err)
	assert.NotNil(t, val)
}
